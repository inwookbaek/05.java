...
/*
	...
	자바5부터 제네릭타입이 새롭게 추가되었다. 제네릭타입을 사용하게 되면 잘못된 타입이 사용되는 가능성을
	컴파일단계부터 제거할 수 있게 되었다. 제네릭은 컬렉션, 람다식, 스트림, NIO(New Input Output)에 널리
	사용되기 때문에 제네릭을 이해하지 못한다면 자바의 API문서등을 이애할 수 없다.
	...
	1. 컴파일시에 강한 타입체크를 할 수 있다.
	   ...
	   에서 강한 타입체크를 한다. 실행시에 타입에러가 발생하는 것 보다 컴파일시에 미리 타입을 강하게 제한
	   해서 에러를 미연에 방지할 수 있다.
	   
	2. 타입변환 제거한다.
	
	   비제네릭코드는 불필요한 타입변환을 하기 때문에 프로그램성능에 악영향을 미치게 된다. 제네릭을 사용하면
	   사전에 정해진 타입으로만 제한 시키기 때문에 불필요한 타입변환을 할 필요가 없어 프로그램의 성능이 향상
	   된다.
	...
	뒤에 다이아몬드부호 "<>"를 붙이고 사이에 타입파라미터가 위치한다. 타입파라미터는 변수명와 동일한 규칙에 따
	라 선언이 가능하지만 일반적으로 대문자 알파벳 한글자로 선언한다.
*/
...
//		로봇.setKind(new TV());
		...
		// 참조타입이 아닌경우
		// roduct<int, String> prod8 = new Product<>(); (x)
