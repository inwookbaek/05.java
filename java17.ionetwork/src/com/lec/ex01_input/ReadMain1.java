...
/*
	...
	InputStream은 바이트기반 입력스트림의 최상위 추상클래스이다. FileInputStream, DataInputStream,
	BufferedInputStream은 모두 InputStream을 상속하고 있다.
	
	InputStream의 주요메서드
	
	1. read() : 입력스트림으로 부터 1byte씩 읽어서 int타입으로 리턴
	2. read(byte[] b) : 입력스트림으로 부터 주어진 byte배열의 크기만큼 읽어서 읽은 바이트수만큼 리턴
...
public class ReadMain1 {
	...
	public static void main(String[] args) throws Exception {
		...
		while(true) {
			read_bytes = is.read(); // 1byte를 읽어서 4byte인 int타입으로 리턴
