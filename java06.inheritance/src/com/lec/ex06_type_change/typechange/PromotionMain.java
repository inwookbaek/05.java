...
/*
	...
	프로그램측면에서 보면 다형성의 하나의 타입에 여러형태의 객체를 대입함으로써 다양한 기능
	을 수행할 수 있게 한다.
	
	다형성을 위해서 Java는 자식클래스가 부모클래스로 타입변환을 허용한다. 즉, 부모타입에 모든
	자식객체들을 대입할 수 있다는 것이다. 이 기능을 이용한다면 모든 객체는 부품화가 가능해 진다.
	
	타입변환이란? 데이터타입을 다른 데이터타입으로 변환하는 행위를 말한다. 기본 데이터타입의
	변환처럼 참조타입(class, array, interface, enum)도 타입변환이 가능하다. 클래스타입변환은
	직접적인 상속관계가 있는 클래스사이에서만 타입변환이 가능하다.
	
	자동타입변환(promotion)은 프로그램실행 도중에 자동적으로 타입변ㄴ환이 일어나는 것을 말한다.
	자동타입변환 개념은 자식은 부모의 특징과 기능을 상속받기 때무넹 부모와 동일하게 취급될 수 
	있다는 것이다.
	
	주의할 점은 자동타입변환이 된 후에는 부모클래스에 선언된 필드와 메서드만 접근이 가능하다.
	비록 변수는 자식객체를 참조하지만 변수로 접근 가능한 멤버는 부모클래스이 멤버로만 제한된다.
	그러나 예외가 있는데 메서드가 자식클래스에서 오버라이딩이 되었다면 부모클래스의 메서드가 
	호출되는 것이 아니라 자식클래스이 메서드가 호출된다.
