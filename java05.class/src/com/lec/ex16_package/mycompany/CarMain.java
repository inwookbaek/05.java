...
/*
	...
	프로그램을 개발하다 보면 객체는 수십개, 많게는 수백개의 클래스를 작성해야 한다.
	...
	포한한 클래스파일명 전체가 클래스 이름이다.
	
	패키지는 클래스를 유일하게 만들어 주는 역할을 한다. 즉, 클래스이파일이름이 동일
	하더래도 패키지가 틀리다면 다른 클래스로 인식한다.
	
	클래스 전체이름은 "패키지명.하위패키지명.클래스명"인데 상위, 하위로 구분이 되어
	있다면 dot(.)구분자로 구분이 된다.
	
	패키지의 선언은 자바의 명명규칙을 준수하는데 관례적으로 패키명은 전부 소문자로 정의
	한다.
*/
