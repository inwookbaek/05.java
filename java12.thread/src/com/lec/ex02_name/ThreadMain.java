...
/*
	...
	메이쓰레드는 "main"이라는 이름을 가지고 있고 개발자가 생성한 쓰레드는 자동적으로
	"Thread-n"이라는 이름으로 설정된다. n은 쓰레드의 생성일련번호를 의미한다.
	자동 부여되는 이름 대신에 개발자가 이름을 부여하고 싶을 경우에는 Thread.setName()
...
public class ThreadMain {
	...
	public static void main(String[] args) {
		...
		System.out.println("작업중인 쓰레드이름 = " + threadA.getName());
		threadA.start();
		
		Thread threadB = new ThreadB();
		System.out.println("작업중인 쓰레드이름 = " + threadB.getName());
		threadB.start();
		
		Thread threadC = new ThreadB();
		System.out.println("작업중인 쓰레드이름 = " + threadC.getName());
		threadC.start();
